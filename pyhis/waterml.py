...
# waterml timeseries parser, inspired by:
# http://www.marine.rutgers.edu/~mayorga/outgoing/waterml_parse_plot.py
# WaterML namespace in the XML file

WMLXMLNS = "http://www.cuahsi.org/waterML/1.0/"

def _ETelfind(root_el, xpathstr, ns, type='one'):
    '''Replace find and findall, to do the namespace substitution.
    Use the string replace method: .replace('/','/{%s}')
    Handle only two cases:
    1, single tag, anywhere in the tree; 2, full path relative to root.
    - Arguments:
        root_el:  ET root element object
        xpathstr: XPath string w/o namespace prefixes
        ns:       namespace URL
        type:     'one' (find()) or 'all' (findall())
    '''

    if type == "one":
        xpathstrns = xpathstr.replace('//','//{%s}')
        return root_el.find(xpathstrns % ns)
    elif type == "all":
        # for now, assumes the element hierarchy is 3-deep;
        # eventually, can parse and count the elements to apply ns substitution
        xpathstrns = xpathstr.replace('/','/{%s}')
        ns_tup = (ns,)*3
        return root_el.findall(xpathstrns % ns_tup)

def timeseries_from_wml_file(file):
    """reads a file or file object in WaterML format and returns a
    pyhis.timeseries element"""
    elvalueall = ETelfind(root_el, './timeSeries/values/value', WMLXMLNS, 'all')
    # convert values to float numpy array
    self.val_a = np.asarray([float(val.text) for val in elvalueall])

    # convert date-time attributes to string list,
    # then to datetime object numpy array.
    # [code/syntax would be much simpler using dateutil.parse()]
    dtstr_lst = [val.get('dateTime') for val in elvalueall]
    # ISO datetime format, for parsing dateTime attribute
    isodtformat = "%Y-%m-%dT%H:%M:%S"
    self.dt_a = NY.asarray([DT.datetime.strptime(dtstr, isodtformat) for dtstr in dtstr_lst])

    # site information
    siteName_el = ETelfind(root_el, './/siteName', WMLXMLNS)
    siteCode_el = ETelfind(root_el, './/siteCode', WMLXMLNS)
    self.siteName     = siteName_el.text
    self.siteCode     = siteCode_el.text
    self.site_network = siteCode_el.get('network')

    # variable information
    variableName_el = ETelfind(root_el, './/variableName', WMLXMLNS)
    variableCode_el = ETelfind(root_el, './/variableCode', WMLXMLNS)
    # units may not be available; returns None if tag is not found
    units_el        = ETelfind(root_el, './/units', WMLXMLNS)
    if units_el == None:
        unitsabbr_str = 'Unknown'
    else:
        unitsabbr_str = units_el.get('unitsAbbreviation')
    self.variableName   = variableName_el.text
    self.variableCode   = variableCode_el.text
    self.var_vocabulary = variableCode_el.get('vocabulary')
    self.var_unitsabbrv = unitsabbr_str

    return pyhis.TimeSeries(
        variable=variableName_el,
        value_count=series.valueCount,
        method='',
